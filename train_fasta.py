...
class FASTADataset(Dataset):
    ...
    def __init__(self, data_dir, k=3, max_seq_length=2048, max_kmers=1000, split='train', train_ratio=0.8, random_state=42):
        ...
        if os.path.exists(covid_dir):
            for fasta_file in os.listdir(covid_dir):
                if fasta_file.endswith('.fasta') and '0.95' in fasta_file:
        ...
        if len(self.labels) > 0:
            print(f"{split.upper()} COVID: {sum(self.labels)}, Non-COVID: {len(self.labels) - sum(self.labels)}")
    ...
    def _encode_sequence_kmers(self, sequence):
        """
        Encode DNA/RNA sequence using K-mer frequency representation
        
        Returns a vector where each element represents the frequency/count
        of a specific K-mer in the sequence.
        """
        ...
        # Normalize by sequence length (optional - can help with varying sequence lengths)
        if len(kmers) > 0:
            kmer_vector = kmer_vector / len(kmers)
